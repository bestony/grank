...
def comsum_owner(owner, config):
    ...
    activity_df = pd.DataFrame({}, index=pd.date_range(
        start=start_time, end=end_time, freq="W"))
    social_df = pd.DataFrame({}, index=pd.date_range(
        start=start_time, end=end_time, freq="W"))
    ...
    for i in range(0, len(list)):
        ...
        if os.path.isfile(path) and os.path.splitext(list[i])[1] == '.pkl' and list[i] != '-ALL-.pkl':
            activity_df = activity_df.add(pd.read_pickle(
                "output/activity/%s/%s" % (owner, list[i])), fill_value=0)

    activity_df["score"] = activity_df.apply(lambda row: math.sqrt(
    ...
    for i in range(0, len(list)):
        ...
        if os.path.isfile(path) and os.path.splitext(list[i])[1] == '.pkl' and list[i] != '-ALL-.pkl':
            social_df = social_df.add(pd.read_pickle(
                "output/social/%s/%s" % (owner, list[i])), fill_value=0)
