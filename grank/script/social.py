...
def analyse_repo(owner, repository, data, config, ret_score=False):
    ...
    for index, row in social_all_frame.iterrows():
        social_all_frame.loc[index, "is_corp"] = helpers.is_corp(row["domain"], config)

    community_df = (
        social_all_frame[social_all_frame.is_corp != True]
        .set_index("date")
        .resample("W")["times"]
        .sum()
    )
    ...
    click.echo(
        "%s/%s 的平均社区化程度为 %.2f%%" % (owner, repository, 100 * target_social_score)
    )
