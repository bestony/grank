...
def analyse_email(data, config):
    ...
    click.echo(df["domain"].value_counts().drop(labels=ignore_mail,
                                                errors="ignore"))

    new_rule = click.prompt("请输入新的社区化识别的正则规则：",
                            default=config["social"]["rule"])
    if new_rule != "":
        if new_rule == "!":
...
def analyse_repo(owner, repository, data, config, ret_score=False):
    ...
    for index, row in social_all_frame.iterrows():
        social_all_frame.loc[index, "domain"] = helpers.detect_email_domain(
            row["author"])

    social_all_frame = social_all_frame[
        (social_all_frame.domain != "")
        & (social_all_frame.domain != "@users.noreply.github.com")
        & (social_all_frame.date != "未标注时间")]
    social_all_frame["date"] = pd.to_datetime(social_all_frame["date"])
    for index, row in social_all_frame.iterrows():
        social_all_frame.loc[index, "is_corp"] = helpers.is_corp(
            row["domain"], config)

    community_df = (social_all_frame[social_all_frame.is_corp != True].
                    set_index("date").resample("W")["times"].sum())
    social_all_df = social_all_frame.set_index("date").resample(
        "W")["times"].sum()

    social_all_df = social_all_df.loc[start_time:end_time]
    community_df = community_df.loc[start_time:end_time]

    temp_community_series = pd.Series(np.zeros((len(date_range), ), dtype=int),
                                      index=date_range)
    temp_social_series = pd.Series(np.zeros((len(date_range), ), dtype=int),
                                   index=date_range)
    ...
    helpers.set_social_average(instance, owner, repository,
                               target_social_score)
